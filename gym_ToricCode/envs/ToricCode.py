...
class ToricCode(gym.Env):
    ...
    def createSyndromOpt(self, tcode):
        ...
        y_errors = (tcode == 2).astype(int, copy=False) # separate y and z errors from x 
        z_errors = (tcode == 3).astype(int, copy=False)
        ...
        charge = (charge == 1).astype(int, copy=False) # annihilate two syndroms at the same place in the grid

        charge = charge[0] + charge[1]
        vertex_matrix = (charge == 1).astype(int, copy=False)
        

        # generate plaquette excitation (flux)
        # can be generated by x and y errors
        x_errors = (tcode == 1).astype(int, copy=False)
        y_errors = (tcode == 2).astype(int, copy=False)
        ...
        flux = (flux == 1).astype(int, copy=False)

        flux = flux[0] + flux[1]
        plaquette_matrix = (flux == 1).astype(int, copy=False)
