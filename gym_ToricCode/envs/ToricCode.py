...
class ToricCode(gym.Env):
    ...
    def step(self, action):
        ...
        self.next_state = self.createSyndromOpt(self.qubit_matrix) # THIS FUNCTION FILLS RAM  
    ...
    def createSyndromOpt(self, tcode):
        ...
        y_errors = (tcode == 2).astype(int) # separate y and z errors from x 
        z_errors = (tcode == 3).astype(int)
        ...
        charge = (charge == 1).astype(int) # annihilate two syndroms at the same place in the grid
        ...
        x_errors = (tcode == 1).astype(int)
        y_errors = (tcode == 2).astype(int)
